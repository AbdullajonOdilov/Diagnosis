...
def all_question_state_answers(search, question_state_option_id,  page, limit, db):
    question_state_answers = db.query(Question_state_answers).options(joinedload(Question_state_answers.user), joinedload(Question_state_answers.question_state_option, ),

                                                )
    if search:
        search_formatted = "%{}%".format(search)
        question_state_answers = question_state_answers.filter(Question_state_answers.answer.like(search_formatted) | Question_state_answers.comment.like(search_formatted))
    else:
        question_state_answers = question_state_answers.filter(Question_state_answers.id > 0)

    if question_state_option_id:
        question_state_answers = question_state_answers.filter(Question_state_answers.question_state_option_id == question_state_option_id)
...
def create_question_state_answer(form, db, thisuser):
    ...
    save_in_db(db=Question_state_answers, obj=new_question_state_db)
